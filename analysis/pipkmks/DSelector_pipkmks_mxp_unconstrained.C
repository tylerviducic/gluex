#include "DSelector_pipkmks_mxp_unconstrained.h"

void DSelector_pipkmks_mxp_unconstrained::Init(TTree *locTree)
{
	...
	dFlatTreeFileName = "pipkmks_mxp_unconstrained.root"; //output flat tree (one combo per tree entry), "" for none
...
Bool_t DSelector_pipkmks_mxp_unconstrained::Process(Long64_t locEntry)
{
	...
	*/
	...
	if(locKinFit_CL < 1E-3){
		...
		locMissingP4_Measured -= locPiPlus1P4_Measured + locKMinusP4_Measured + locPiPlus2P4_Measured + locPiMinusP4_Measured;
		TLorentzVector locPipKmKs = locPiPlus1P4 + locPiPlus2P4 + locKMinusP4 + locPiMinusP4;
		...
		*/
		...
		double minus_t = -1 * (locBeamP4 - locPipKmKs).M2();
...
void DSelector_pipkmks_mxp_unconstrained::Finalize(void)
{
