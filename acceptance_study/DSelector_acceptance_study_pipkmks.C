#include "DSelector_acceptance_study_pipkmks.h"

void DSelector_acceptance_study_pipkmks::Init(TTree *locTree)
{
	...
	dOutputFileName = "acceptance_study_pipkmks.root"; //"" for none
	...
	dHist_None = new TH1F("None", ";None", 100, 0.0, 1.0);
	dHist_CL_Only = new TH1F("CL_Only", ";CL_Only", 100, 0.0, 1.0);
	dHist_FlightSig = new TH1F("FlightSig", ";Flight Sig", 100, 0.0, 1.0);
	dHist_Vertex = new TH1F("Vertex", ";Vertex", 100, 0.0, 1.0);
	dHist_Colin = new TH1F("Colin", ";Colin", 100, 0.0, 1.0);
	dHist_Colin_Vertex = new TH1F("Colin_Vertex", ";Colin_Vertex", 100, 0.0, 1.0);
	dHist_Colin_FlightSig = new TH1F("Colin_FlightSig", ";Colin_FlightSig", 100, 0.0, 1.0);
	dHist_FlightSig_Vertex = new TH1F("FlightSig_Vertex", ";FlightSig_Vertex", 100, 0.0, 1.0);
	dHist_Colin_FlightSig_Vertex = new TH1F("Colin_FlightSig_Vertex", ";Colin_FlightSig_Vertex", 100, 0.0, 1.0);
	...
	*/
	...
	// dFlatTreeInterface->Create_Branch_Fundamental<Double_t>("colinearity_angle");
	// dFlatTreeInterface->Create_Branch_Fundamental<Double_t>("flight_signifigance");
	// dFlatTreeInterface->Create_Branch_Fundamental<Double_t>("vertex_separation");
...
Bool_t DSelector_acceptance_study_pipkmks::Process(Long64_t locEntry)
{
	...
	set<set<Int_t> > locUsedSoFar_IM_Ks;
	...
	*/
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
		...
		locMissingP4_Measured -= locPiPlus1P4_Measured + locKMinusP4_Measured + locProtonP4_Measured + locPiPlus2P4_Measured + locPiMinusP4_Measured;
		...
		*/
		...
		if(cos_theta_col > 0.99){
			dHist_Colin->Fill(locKs.M());
		}

		if(vertex_distance_Ks > 3.0){
			dHist_Vertex->Fill(locKs.M());
		}

		if(locPathLengthSignificanceKs > 5){
			dHist_FlightSig->Fill(locKs.M());
		}

		if(cos_theta_col > 0.99 && vertex_distance_Ks < 3.0){
			dHist_Colin_Vertex->Fill(locKs.M());
		}

		if(cos_theta_col > 0.99 && locPathLengthSignificanceKs > 5){
			dHist_Colin_FlightSig->Fill(locKs.M());
		}

		if(vertex_distance_Ks > 3 && locPathLengthSignificanceKs > 5){
			dHist_FlightSig_Vertex->Fill(locKs.M());
		}

		if(vertex_distance_Ks > 3 && cos_theta_col > 0.99){
			dHist_Colin_FlightSig->Fill(locKs.M());
		}

		if(vertex_distance_Ks > 3 && cos_theta_col > 0.99 && locPathLengthSignificanceKs > 5){
			dHist_Colin_FlightSig_Vertex->Fill(locKs.M());
		}
...
void DSelector_acceptance_study_pipkmks::Finalize(void)
{
