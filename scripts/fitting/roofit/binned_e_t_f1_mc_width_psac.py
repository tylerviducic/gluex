# fit acceptance corrected signal MC KKPi in E and t bins for cross section to get the width used in the fits to data
...
#     voight_width_error = ct.F1_PIPKMKS_VOIGHT_WIDTH_ERROR
    voight_resolution = ct.F1_PIPKMKS_ACCEPTANCE_CORRECTED_VOIGHT_SIGMA
...
#     voight_width_error = ct.F1_PIMKPKS_VOIGHT_WIDTH_ERROR
    voight_resolution = ct.F1_PIMKPKS_ACCEPTANCE_CORRECTED_VOIGHT_SIGMA
...
for e in range(7, 12):
    for t in range(1, 8):
        c1.cd(t)
        signal_mc_hist = ct.accepptance_correct_all_gluex1_kkpi_signal_mc_with_phasespace(channel, cut, e, t)
        ...
        frame.Draw()
        c1.Update()
    c1.SaveAs(f'/work/halld/home/viducic/plots/thesis/binned_resolution_fits/psac_{channel}_binned_resolution_fits_beam_{e}_t_{ct.T_BIN_DICT[t]}.png')
...
df.to_csv(f'/work/halld/home/viducic/data/fit_params/{channel}/psac_binned_e_t_f1_mc_width.csv', index=False)
