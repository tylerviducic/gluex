...
hist_pipkmks = df_recon_filtered.Histo1D((channel, f'{channel}_m', 200, 1.1, 1.5), f'{channel}_m')
hist_t = df_recon_filtered.Histo1D(('mand_t', 'mand_t', 200, 0.1, 2), 'mand_t')
hist_ks = df_recon_no_cuts.Filter(kcuts.KS_PATHLENGTH_CUT).Filter(kcuts.KMP_MASS_CUT).Filter(kcuts.KSP_MASS_CUT)\
    .Filter(kcuts.PPIP_MASS_CUT).Filter(kcuts.P_P_CUT).Histo1D(('ks_m', 'ks_m', 200, 0.4, 0.6), 'ks_m')
