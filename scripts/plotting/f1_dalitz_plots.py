...
hist_ks_km = df.Histo2D(("dalitz_ks_km", "Dalitz Plot for KsPi+ vs KsK-", 50, 0.9, 1.3, 50, 0.4, 0.7), "dalitz_ks_km_m2", "dalitz_ks_pip1_m2")
hist_ks_km.GetXaxis().SetTitle("KsK- Invariant Mass Squared (GeV^{2}/c^{4})")
hist_ks_km.GetYaxis().SetTitle("KsPi+ Invariant Mass Squared (GeV^{2}/c^{4})")
hist_ks_pip = df.Histo2D(("dalitz_ks_pip1", "Dalitz Plot for K-Pi+ vs KsK-", 50, 0.9, 1.3, 50, 0.4, 0.7), "dalitz_ks_km_m2", "dalitz_km_pip1_m2")
hist_ks_pip.GetXaxis().SetTitle("KsK- Invariant Mass Squared (GeV^{2}/c^{4})")
hist_ks_pip.GetYaxis().SetTitle("K-Pi+ Invariant Mass Squared (GeV^{2}/c^{4})")
hist_km_pip = df.Histo2D(("dalitz_km_pip1", "Dalitz Plot for K-Pi+ vs KsPi+", 50, 0.4, 0.7, 50, 0.4, 0.7), "dalitz_ks_pip1_m2", "dalitz_km_pip1_m2")
hist_km_pip.GetXaxis().SetTitle("K-Pi+ Invariant Mass Squared (GeV^{2}/c^{4})")
hist_km_pip.GetYaxis().SetTitle("KsPi+ Invariant Mass Squared (GeV^{2}/c^{4})")
