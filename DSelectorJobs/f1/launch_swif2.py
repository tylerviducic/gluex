...
def add_job(WORKFLOW, FILEPATH, config_dict):
	...
	if(VERBOSE == True):
		scriptOnlyCommand = config_dict["SCRIPTFILE"] + " " + config_dict["ENVFILE"] + " " + FILENAME + " " + JANA_CONFIG + " " + config_dict["OUTDIR_LARGE"] + " " + config_dict["OUTDIR_SMALL"] + " " + RUNNO + " " + FILENO + " " + CACHE_PIN_DAYS
		scriptOnlyCommand += " " + config_dict["ROOT_SCRIPT"] + " " + config_dict["TREE_NAME"] + " " + config_dict["SELECTOR_NAME"] + " " + config_dict["NCORES"]
		print "job add command is \n" + str(add_command)
		if(INDATA_DIR[:5] == "/mss/"): print "\nINTERACTIVE COMMAND1: \n cp /cache/" + FILEPATH[5:] + " ./ "
		else:                          print "\nINTERACTIVE COMMAND1: \n cp " + FILEPATH + " ./ "
		print "INTERACTIVE COMMAND2: \n " + scriptOnlyCommand
...
def main(argv):
	...
	# print "CURRENT CONFIG: " + str(config_dict)
	# sys.exit()
