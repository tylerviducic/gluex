#include "DSelector_mc_pimkpks_recon.h"

void DSelector_mc_pimkpks_recon::Init(TTree *locTree)
{ 
	...
	dOutputFileName = "mc_pimkpks_recon_spring.root"; //"" for none 
	...
	dHist_IM_KpP = new TH1F("IM_KpP", ";IM(K+ p) GeV", 100, 1.0, 2.5);
	...
	dHist_W_vs_IM_KsKpPim = new TH2F("W_vs_IM_KsKpPim", "W vs IM(Ks K- pi+)", 150, 1.0, 2.2, 100, 3, 5);
...
//low t range
	for(int i = 0; i < 4; i++){
		...
		dHist_IM_KsKmPip_binned_full[i] = new TH1F(hist_name4, hist_title, 70, 1, 1.7);
		dHist_f1_1285_binned_full[i] = new TH1F(hist_name5, hist_title, 70, 1, 1.7);

		dHist_f1_1285_beam_5[i] = new TH1F(hist_name_b11, hist_title, 70, 1, 1.7);
		dHist_f1_1285_beam_6[i] = new TH1F(hist_name_b12, hist_title, 70, 1, 1.7);
		dHist_f1_1285_beam_7[i] = new TH1F(hist_name_b7, hist_title, 70, 1, 1.7);
		dHist_f1_1285_beam_8[i] = new TH1F(hist_name_b8, hist_title, 70, 1, 1.7);
		dHist_f1_1285_beam_9[i] = new TH1F(hist_name_b9, hist_title, 70, 1, 1.7);
		dHist_f1_1285_beam_10[i] = new TH1F(hist_name_b10, hist_title, 70, 1, 1.7);
	...
	for(int i = 0; i < 2; i++){
		...
		dHist_IM_KsKpPim_binned_full_med[i] = new TH1F(hist_name4, hist_title, 70, 1, 1.7);
	...
	for(int i = 0; i < 2; i++){
		...
		dHist_IM_KsKpPim_binned_full_high[i] = new TH1F(hist_name4, hist_title, 70, 1, 1.7);
...
Bool_t DSelector_mc_pimkpks_recon::Process(Long64_t locEntry)
{
	...
	set<set<Int_t> > locUsedSoFar_IM_KpP;
...
void DSelector_mc_pimkpks_recon::Finalize(void)
{
