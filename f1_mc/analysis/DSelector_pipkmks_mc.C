#include "DSelector_pipkmks_mc.h"

void DSelector_pipkmks_mc::Init(TTree *locTree)
{
	...
	dOutputFileName = "pipkmks_mc.root"; //"" for none
...
Bool_t DSelector_pipkmks_mc::Process(Long64_t locEntry)
{
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
		...
		*/
		...
		if (locUsedSoFar_PipKmKs.find(locUsedThisCombo_PipKmKs) == locUsedSoFar_PipKmKs.end())
		{
			...
			if (minus_t < 2)
			{
				dHist_f1_1285_binned_full[t_index]->Fill(im_pipkmks, locHistAccidWeightFactor);
				if (beam_energy > 6.5 && beam_energy < 7.5)
				{
					dHist_f1_1285_beam_7[t_index]->Fill(im_pipkmks, locHistAccidWeightFactor);
				}
				if (beam_energy > 7.5 && beam_energy < 8.5)
				{
					dHist_f1_1285_beam_8[t_index]->Fill(im_pipkmks, locHistAccidWeightFactor);
				}
				if (beam_energy > 8.5 && beam_energy < 9.5)
				{
					dHist_f1_1285_beam_9[t_index]->Fill(im_pipkmks, locHistAccidWeightFactor);
				}
				if (beam_energy > 9.5 && beam_energy < 10.5)
				{
					dHist_f1_1285_beam_10[t_index]->Fill(im_pipkmks, locHistAccidWeightFactor);
				}
			}
...
void DSelector_pipkmks_mc::Finalize(void)
{
